...
public class EditTextWithDel extends EditText {
    ...
    private void init() {
        imgAble = mContext.getResources().getDrawable(R.drawable.ico_delete);
    ...
    private void setDrawable() {
        ...
        if (length() > 0) {
            setCompoundDrawablesWithIntrinsicBounds(dr[0], dr[1], imgAble,dr[3]);
        } else {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (imgAble != null && event.getAction() == MotionEvent.ACTION_UP) {
            ...
            rect.left = rect.right - imgAble.getIntrinsicWidth();
