...
public class FileHelper {
    ...
    public static String getRealPath(Uri uri, Context cordova) {
        String realPath = null;

        if (Build.VERSION.SDK_INT < 11)
            realPath = FileHelper.getRealPathFromURI_BelowAPI11(cordova, uri);

            // SDK >= 11 && SDK < 19
        else if (Build.VERSION.SDK_INT < 19)
            realPath = FileHelper.getRealPathFromURI_API11to18(cordova, uri);

            // SDK > 19 (Android 4.4)
        else
            realPath = FileHelper.getRealPathFromURI_API19(cordova, uri);
     ...
     * @return the full path to the file
     */
    public static String getRealPath(String uriString, Context cordova) {
    ...
    public static String getRealPathFromURI_API19(Context context, Uri uri) {
        ...
        try {
            ...
            String id = wholeID.indexOf(":") > -1 ? wholeID.split(":")[1]
                    : wholeID.indexOf(";") > -1 ? wholeID.split(";")[1]
                    : wholeID;
    ...
    public static InputStream getInputStreamFromUriString(String uriString,
                                                          Context cordova) throws IOException {
        ...
        if (uriString.startsWith("content")) {
            ...
            if (uriString.startsWith("file:///android_asset/")) {
                ...
                try {
                    returnValue = cordova.getContentResolver().openInputStream(
                            Uri.parse(uriString));
                } catch (Exception e) {
                    returnValue = null;
                }
                if (returnValue == null) {
                    returnValue = new FileInputStream(getRealPath(uriString,
                            cordova));
                }
            }
        } else {
